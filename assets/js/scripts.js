// ────────────────────────────────────────────────────────────────
        ...
        let recordingSaveModalOpen = false;
        ...
        const recordingSaveModal = document.getElementById('recordingSaveModal');
        const recordingSaveContent = document.getElementById('recordingSaveContent');
        const recordingPlaylistList = document.getElementById('recordingPlaylistList');
        ...
        // ────────────────────────────────────────────────────────────────
        //  HTML ENTITY DECODER / ENCODER (Security)
        // ────────────────────────────────────────────────────────────────
        ...
        // ────────────────────────────────────────────────────────────────
        //  PWA INSTALLATION & APP DETECTION
        // ────────────────────────────────────────────────────────────────
        let deferredPrompt;
        ...
        // ────────────────────────────────────────────────────────────────
        const tourSteps = [
            { title: "Welcome to Song!", desc: "Thank you for choosing Song. Let's take a quick tour to help you get started." },
            { title: "🏠 Home", desc: "Discover new music, curated playlists, and trending tracks right here on the Home tab." },
            { title: "🔍 Search", desc: "Search for songs, albums, artists, or playlists using the Search tab." },
            { title: "📋 Playlists", desc: "Create and manage your own playlists, and access your Favorites and Downloads." },
            { title: "▶️ Queue", desc: "View and reorder your current playback queue here." },
            { title: "✨ For You", desc: "Get AI-powered recommendations based on your listening history and favorites." },
            { title: "💿 Mini Player", desc: "The floating circular player shows the current song. Drag it anywhere on the screen." },
            { title: "🎵 Full Player", desc: "Tap the circular player to open the full player modal with all controls, visualizer, and more!" }
        ];
        let currentTourStep = 0;

        function startTour() {
            if (localStorage.getItem('songAppToured')) return;
            tourModal.classList.add('open');
            currentTourStep = 0;
            updateTourStep();
            // Prevent overlay close on click for tour
            tourModal.onclick = null;
        }
        ...
        function updateTourStep() {
            ...
            for (let i = 0; i < tourSteps.length; i++) {
                ...
                dot.className = 'dot' + (i === currentTourStep ? ' active' : '');
                tourProgress.appendChild(dot);
            }
            if (currentTourStep === tourSteps.length - 1) {
                tourNextBtn.textContent = 'Finish';
                tourNextBtn.onclick = endTour;
            } else {
        ...
        function endTour() {
            ...
            localStorage.setItem('songAppToured', 'true');
            notify('🎉 Tour completed! Enjoy using Song.', 'success');
        }
        ...
        function confirmCreatePlaylist() {
            ...
            viewPlaylist(id);
            closePlaylistModal();
            if (recordingSaveModalOpen) {
                renderRecordingPlaylistList(recordedTrack);
            }
        ...
        function renderTrackList(tracks, container, isSearch = false, listId = 'queue') {
            ...
            tracks.forEach((track, idx) => {
                ...
                // Delete button for Favorites and Downloads
                if ((listId === 'favorites' || listId === 'downloads') && !isSearch) {
                    const deleteAction = listId === 'favorites' 
                        ? `toggleFavorite(${trackJSON})` 
                        : `deleteDownload('${track.id}')`;
                    actions += `
                        <button class="btn-delete" onclick="event.stopPropagation(); ${deleteAction}" title="Remove">
                            <i class="fa-solid fa-trash"></i>
                        </button>`;
                }
                // Delete button for custom playlists
                if (!isSearch && listId !== 'queue' && listId !== 'favorites' && listId !== 'downloads') {
                    actions += `
        ...
        function updatePlayerUI() {
            ...
            // If visualizer is active, ensure canvas is showing
            if (isVisualizerActive) {
                visualizerCanvas.style.display = 'block';
                modalArt.style.opacity = '0.3';
                visualizerBtn.style.color = '#8b5cf6';
            }
        }
        ...
        // ────────────────────────────────────────────────────────────────
        //  VISUALIZER (Replaces Lyrics Button)
        // ────────────────────────────────────────────────────────────────
            ...
            if (isVisualizerActive) {
                if (!audioContext) {
                    try {
                        ...
                        notify('Visualizer not supported or audio not playing.', 'warning');
                ...
                modalArt.style.opacity = '1';
                visualizerBtn.style.color = 'inherit';
        ...
        // ────────────────────────────────────────────────────────────────
        //  RECORDING FEATURE WITH MODAL & POLYGRAPH VISUALIZER
        // ────────────────────────────────────────────────────────────────
            ...
            try {
                ...
                mediaRecorder.onstop = async () => {
                    ...
                    tempAudio.addEventListener('loadedmetadata', () => {
                        recordedTrack.duration = Math.round(tempAudio.duration);
                        // Close recording modal and open save modal
                        closeRecordingModal();
                        openRecordingSaveModal(recordedTrack);
                    });
                    audioChunks = [];
                };
        ...
        function stopRecording() {
            ...
            // Don't close modal here, it will close in onstop
        }

        function openRecordingModal() {
            recordingModal.classList.add('open');
            recordingModalOpen = true;
            // Start polygraph visualizer
            const bars = recordingVisualizer;
            bars.innerHTML = '';
            for (let i = 0; i < 30; i++) {
        ...
        function animateRecordingBars() {
            const bars = recordingVisualizer.querySelectorAll('.bar');
            if (!recordingModalOpen) return;
            bars.forEach(bar => {
                const height = Math.random() * 80 + 10;
                bar.style.height = height + 'px';
            });
            recordingAnimFrame = requestAnimationFrame(animateRecordingBars);
        }

        function openRecordingSaveModal(track) {
            recordingSaveModal.classList.add('open');
            recordingSaveModalOpen = true;
            renderRecordingPlaylistList(track);
        }

        function closeRecordingSaveModal() {
            recordingSaveModal.classList.remove('open');
            recordingSaveModalOpen = false;
            recordedTrack = null;
        }

        function renderRecordingPlaylistList(track) {
            const list = document.getElementById('recordingPlaylistList');
            list.innerHTML = '';
            Object.keys(playlists).forEach(id => {
                const pl = playlists[id];
                const div = document.createElement('div');
                div.style.cssText = 'display:flex;align-items:center;gap:12px;padding:8px 12px;background:rgba(255,255,255,0.04);border-radius:8px;cursor:pointer;';
                div.innerHTML = `
                    <span><i class="fa-solid fa-list"></i></span>
                    <span style="flex:1;">${pl.name}</span>
                `;
                div.onclick = () => {
                    saveRecordingToPlaylist(track, pl.id);
                };
                list.appendChild(div);
            });
        }

        function createPlaylistForRecording() {
            openPlaylistModal();
            // The modal will handle creation and then call renderRecordingPlaylistList
        }

        function saveRecordingToPlaylist(track, playlistId) {
            // Save to IndexedDB vault
            saveDownload(track).then(() => {
                // Add to playlist
                addTrackToPlaylist(playlistId, track);
                closeRecordingSaveModal();
                notify(`✅ Recording saved to "${playlists[playlistId].name}" and offline vault!`, 'success');
            });
        }
        ...
        async function generateForYouRecs() {
            ...
            const historySec = document.getElementById('fyListeningHistorySection');
            const historyList = document.getElementById('fyListeningHistoryList');
            if (listeningHistory.length > 0) {
                historySec.style.display = 'block';
                renderTrackList(listeningHistory.slice(0, 10), historyList, true);
            } else {
        ...
        async function deleteDownload(id) {
            ...
            if (playlists.downloads) {
                ...
                savePlaylists();
                renderPlaylists();
                if (currentViewPlaylistId === 'downloads') viewPlaylist('downloads');
            }
        }
        ...
        async function init() {
            ...
            if (isStandalone()) {
                ...
                // User is in browser, but we already show the install button
                // Optionally show a notification
                notify('💡 Tip: You can install this app to your home screen for a better experience.', 'info');
            }
